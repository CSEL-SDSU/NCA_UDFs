...
*/
// Modified 7/2/2026 to support different gap heights
DEFINE_PROFILE(inlet_x_vel_rpvar, thread, position)
{
	real x[ND_ND]; /* this will hold the position vector */
	real y, h, U_mean, U_max, m, n = 2.0;
	face_t f;

	h = 0.00495; /* m; inlet height, do not change */

	bool h_exists = RP_Variable_Exists_P("user/h"); // Check if user-defined parameter for inlet height exists
	Message0("Checking for user-defined parameter 'user/h': %d\n", h_exists);

	if (RP_Variable_Exists_P("user/h"))
	{
		h = RP_Get_Real("user/h"); // Get inlet height from user-defined parameter if it exists

		real a = 0.0415; // half width of narrow channel [m]
		real b = h/2; // half height of narrow channel [m]

		real a_star = 2 * b / 2 * a; //nca aspect ratio
		// compute correction factor
		m = 1.7 + 0.5 * pow(a_star, -1.4); 
	}
	else
	{
		Message0("Warning: User-defined parameter 'user/h' not found. Using default value of %f m.\n", h);
		m = 27.59596236; /* constant, do not change, case for 4.95 mm gap */
	}
...
*/
// Modified for 10 mm gap case with only one surface with chemical reaction. 
...
#if !RP_HOST // Run on nodes in parallel or on single process in serial 
	// Find wall_mass_flux thread
	Domain* d = Get_Domain(1); // Get domain pointer, update if different
	int zone_ID = 8; // ID of surface zone where chemical reaction occurs, update if different. Zone is shown in Boundary conditions tab
...
// Setting and checking RP variables 
DEFINE_ON_DEMAND(check_u_mean)
{
	...
	//Message("Checking for user-defined parameter 'user/alpha': %d\n", alpha_exists);

	if (U_mean_exists)
	{
		real U_mean = RP_Get_Real("user/u_mean"); // Get mean velocity from user-defined parameter if it exists
		Message("User-defined parameter 'user/u_mean' found with value: %f m/s\n", U_mean);
	}
	else
	{
		Message("Warning: User-defined parameter 'user/u_mean' not found.\n");
	}
#endif

}

DEFINE_ON_DEMAND(check_h)
{
#if !RP_NODE
	bool h_exists = RP_Variable_Exists_P("user/h"); // Check if user-defined parameter for mean velocity exists
	//bool alpha_exists = RP_Variable_Exists_P("user/alpha"); // Check if user-defined parameter for under-relaxation factor exists

	Message("Checking for user-defined parameter 'user/h': %d\n", h_exists);
	//Message("Checking for user-defined parameter 'user/alpha': %d\n", alpha_exists);

	if (h_exists)
	{
		real h = RP_Get_Real("user/h"); // Get mean velocity from user-defined parameter if it exists
		Message("User-defined parameter 'user/h' found with value: %f m\n", h);
	}
	else
	{
		Message("Warning: User-defined parameter 'user/h' not found.\n");
	}
#endif
...
DEFINE_ON_DEMAND(set_FSR)
{
...
#if !RP_NODE //Run on host in parallel or on single process in serial. Only host talks with scheme
	...
	Message0("Printing V_f on all nodes: \n");
	Message("V_f initialized to %g m/s \n", V_f);
}

DEFINE_ON_DEMAND(set_alpha)
{
#if !RP_NODE //Run on host in parallel or on single process in serial. Only host talks with scheme

	bool alpha_exists = RP_Variable_Exists_P("user/alpha"); // Check if user-defined parameter for alpha exists)

	Message("Checking for user-defined parameter 'user/alpha': %d\n", alpha_exists);

	if (alpha_exists)
	{
		alpha = RP_Get_Real("user/alpha"); // Get initial FSR from user-defined parameter if it exists
		Message("User-defined parameter 'user/alpha' found with value: %g \n", alpha);
	}
	else
	{
		alpha = 1; // Default alpha value if user-defined parameter does not exist
		Message("Warning: User-defined parameter 'user/alpha' not found. Using default value of 1.\n");
	}
#endif

	//node_to_host_real_1(V_f); // update V_f on host process so report is correct.
	host_to_node_real_1(alpha);
	Message0("Printing alpha on all nodes: \n");
	Message("alpha initialized to %g \n", alpha);
